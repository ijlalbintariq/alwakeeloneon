...
**Stack**: React + TypeScript (frontend) · Express.js + TypeScript (backend) · PostgreSQL + Drizzle ORM · OpenRouter AI · Kimi K2.5 (Apex)
...
- A chat interface where users consult an AI legal advisor (OpenRouter / Kimi K2.5)
...
| Primary AI | OpenRouter API (OpenAI-compatible SDK) |
...
- Cost rates are estimated per model via OpenRouter pricing
...
### 8.1 OpenRouter (Primary AI Provider)
...
**Model selection**: OpenRouter dynamically routes to whatever free model is available. The default model ID is configured as a constant but OpenRouter handles actual model selection based on availability.

**Used for**: All AI features — chat, judgment search, statute search, legal drafting, contract drafting, briefs, summaries, audio transcription.
...
data: {"done": true, "model": "google/gemini-2.0-flash-001"}\n\n
```
...
Cost rates are estimated based on the OpenRouter model used. Rates are configured in the `MODEL_COSTS` map in `server/routes.ts`.
...
#   2. esbuild → dist/server.js (backend bundle)
```
