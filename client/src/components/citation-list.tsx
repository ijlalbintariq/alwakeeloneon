    ...
    return (
      <div className="citation-suggestion-popup">
        <div className="flex items-center gap-2 px-3 py-4 text-muted-foreground text-xs">
          ...
          Type after <code className="bg-card/60 px-1 py-0.5 rounded text-[10px] font-mono">/cite</code> to search judgments...
        </div>
      </div>
    );
  }
      ...
      <div className="max-h-64 overflow-y-auto py-1">
        {items.map((item, index) => (
          <button
            ...
            className={`w-full text-left px-2.5 py-2 flex flex-col gap-0.5 transition-colors ${
              index === selectedIndex
          ...
          >
            ...
            <span className="text-[11px] text-foreground/80 line-clamp-1 leading-tight">
              {item.title}
            </span>
          </button>
        ))}
