...
export const ChatHistoryDrawer: React.FC<ChatHistoryDrawerProps> = ({
  ...
  return (
    <aside
      className={cn(
        "flex flex-col h-full bg-white transition-all duration-300 ease-in-out shrink-0 overflow-hidden relative select-none z-20",
        isOpen ? "w-72 md:w-80 border-r border-[#E2E8F0] shadow-md" : "w-0 border-none pointer-events-none"
      )}
    >
      {/* Header & Toggle */}
      <div className="p-3.5 border-b border-[#E2E8F0] flex items-center justify-between gap-2 min-h-[57px] bg-[#F8FAFC]">
        {isOpen && (
          <>
            <div className="flex items-center gap-2 min-w-0">
              <div className="h-7 w-7 rounded-lg bg-[#105B38]/10 text-[#105B38] border border-[#105B38]/20 flex items-center justify-center shrink-0">
      ...
      {isOpen && (
        <>
          ...
          <div className="p-3 border-b border-[#E2E8F0]">
            <div className="relative flex items-center">
              ...
              <input
                ...
                className="w-full bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl pl-8 pr-3 py-1.5 text-xs text-[#0F172A] placeholder:text-[#94A3B8] focus:outline-none focus:border-[#105B38] transition-colors shadow-xs"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1 scrollbar-thin scrollbar-thumb-[#CBD5E1]">
            {isLoading ? (
              <div className="py-8 text-center text-xs text-[#64748B] space-y-2">
                <div className="w-5 h-5 border-2 border-[#E2E8F0] border-t-[#105B38] rounded-full animate-spin mx-auto" />
                <p>Loading consultation history...</p>
              </div>
            ) : filteredThreads.length === 0 ? (
              <div className="py-8 text-center text-xs text-[#64748B] space-y-1.5 px-4">
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isActive
                        ? "bg-emerald-50/80 text-[#105B38] border-emerald-200 shadow-xs font-semibold"
                  ...
                  >
                    ...
                    {onDeleteThread && (
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-[#F5F4F2] text-[#999999] hover:text-[#1A1A1A] transition-all"
          ...
          {/* Footer Badge */}
          <div className="p-3 border-t border-[#E5E4E2] bg-white text-[11px] text-[#666666] flex items-center justify-between">
            <div className="flex items-center gap-1.5 text-[#666666] font-mono text-[10px]">
              <ShieldCheck className="w-3.5 h-3.5 text-[#1A1A1A]" />
              <span>Privileged & Encrypted</span>
            </div>
            <span className="text-[10px] text-[#999999] font-mono">Chambers Vault</span>
          </div>
        </>
      )}
      ...
      {!isOpen && (
        <div className="flex-1 flex flex-col items-center py-4 space-y-4">
          <button
            onClick={onNewConsultation}
            className="p-2 rounded-lg bg-white text-[#1A1A1A] hover:bg-[#F5F4F2] border border-[#E5E4E2] transition-colors shadow-sm"
          ...
          <div
            className="text-[10px] text-[#999999] font-mono uppercase tracking-widest -rotate-90 origin-center whitespace-nowrap mt-8"
            style={{ width: "80px" }}
          >
            History
          </div>
