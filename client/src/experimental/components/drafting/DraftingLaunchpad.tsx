...
export const DraftingLaunchpad: React.FC<DraftingLaunchpadProps> = ({
  ...
  return (
    <div className="w-full h-full overflow-y-auto bg-[#F8FAFC] p-4 sm:p-6 lg:p-8 custom-scrollbar">
      <div className="max-w-6xl mx-auto space-y-6">
        {/* ── Top Header ────────────────────────────────────────────────────── */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-6 rounded-2xl border border-[#E2E8F0] shadow-xs">
          <div>
          ...
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            {onBackToEditor && (
              <button
                ...
                className="inline-flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-white hover:bg-[#F1F5F9] border border-[#CBD5E1] text-xs font-bold text-[#0F172A] transition-all shadow-xs"
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#F8FAFC] hover:bg-[#F1F5F9] border border-[#E2E8F0] text-xs font-bold text-[#334155] transition-all shadow-xs"
            >
              <FilePlus className="w-4 h-4 text-[#64748B]" />
        ...
        <div className="bg-white rounded-2xl border border-emerald-200/80 shadow-xs overflow-hidden">
          ...
          <form onSubmit={handleLaunchAiBrief} className="p-6 space-y-4">
            <div className="flex gap-2 mb-4 bg-[#F8FAFC] p-1 rounded-lg w-fit border border-[#E2E8F0]">
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full h-10 px-3 rounded-xl bg-white border border-[#E2E8F0] text-xs font-medium text-[#0F172A] placeholder:text-[#94A3B8] focus:outline-none focus:border-[#105B38]"
                />
              </div>
            ...
            <div>
              <label className="text-xs font-bold text-[#0F172A] flex items-center justify-between mb-1">
                <span>{draftType === "pleading" ? "Factual Matrix / Brief Facts" : "Key Terms & Obligations"}</span>
                <span className="text-[10px] font-medium text-[#94A3B8]">Shift + Enter for new line</span>
              </label>
              <textarea
                ...
                rows={2}
                className="w-full p-3 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] text-xs text-[#0F172A] placeholder:text-[#94A3B8] focus:outline-none focus:border-[#105B38] resize-none"
              />
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="relative w-full sm:w-72">
              ...
              <input
                ...
                className="w-full h-9 pl-9 pr-3 rounded-xl bg-white border border-[#E2E8F0] text-xs text-[#0F172A] placeholder:text-[#94A3B8] focus:outline-none focus:border-[#105B38]"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5">
            {filteredTemplates.map((template) => (
              <div
                ...
                className="group p-4 rounded-2xl bg-white border border-[#E2E8F0] hover:border-[#105B38] hover:shadow-md transition-all cursor-pointer flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-[#105B38] border border-emerald-200">
                      {template.category}
                    </span>
                  ...
                  <h3 className="text-xs font-bold text-[#0F172A] group-hover:text-[#105B38] transition-colors leading-snug line-clamp-2">
                    {template.title}
                  </h3>

                  <p className="text-[11px] text-[#64748B] font-serif line-clamp-2 mt-2 leading-relaxed">
