...
import { apiRequest, AppError } from "@/lib/queryClient";
    ...
    try {
      ...
      try {
        ...
        const conversationHistory = messages
          ...
          .map((m) => ({
            role: m.role,
            // Assistant turns carry only the explanation. The draft body is sent
            // separately as draftText; echoing it here made the model restore stale drafts.
            content: m.role === "assistant" && m.insertableClause
        ...
        if (aiContextFiles.length > 0) {
          ...
          if (!res.ok) {
            ...
            throw new AppError(errBody.message || "Failed to generate draft", res.status, res.status === 422 ? errBody : undefined);
          }
        } else {
          const payload = {
            prompt: query,
        ...
        if (err instanceof AppError) {
          ...
          if (err.status === 422) {
            // 422 means the server REFUSED the edit and handed back the draft
            // unchanged. Rendering it as a normal result showed an Apply button
            // on identical text and hid the reason, so report it instead.
            data = err.data;
            if (!data) throw err;
            data = { ...data, rejected: true };
          } else {
            throw err;
          }
        } else {
      ...
      // Analysis answers are commentary, not draft content. Offering Replace on
      // one let a click overwrite the pleading with an essay.
      if (data.assistantMode === "analysis" || data.operation === "answer") {
        setMessages((prev) => [...prev, {
          id: `ai-analysis-${Date.now()}`,
          role: "assistant",
          text: data.clause || data.message || "No analysis returned.",
          timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
          recommendations,
        }]);
        return;
      ...
      // For an edit the server returns the FULL patched draft, not a snippet.
      // Inserting that at the cursor would duplicate the whole pleading, so
      // those responses offer Replace only, and the server's own summary
      // ("Updated GROUND D only...") is shown instead of the generic line.
      const WHOLE_DOCUMENT_OPERATIONS = new Set([
        "section-edit",
        "targeted-edit",
        "full-rewrite",
        "conversion",
      ...
      const isWholeDocument = WHOLE_DOCUMENT_OPERATIONS.has(String(data.operation || ""));
      const explanation =
        data.assistantMessage ||
        data.explanation ||
        data.summary ||
        "Here is the court-ready legal draft generated by the AI legal model:";
      const advisories: string[] = Array.isArray(data.advisories) ? data.advisories : [];
      const clauseTitle =
        data.title ||
        (data.changedTarget ? `Updated draft — ${data.changedTarget}` : null) ||
        (isWholeDocument ? "Updated Draft" : "Court-Ready Clause");
      const explanationWithAdvisories = advisories.length
      ...
      const aiMsg: ChatMessage = {
        id: `ai-${Date.now()}`,
        role: "assistant",
        text: explanationWithAdvisories,
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            ...
                            className={cn(
                              ...
                              msg.isWholeDocument
                                ? "px-2 py-0.5 bg-[#105B38] hover:bg-[#0D4A2E] text-white shadow-xs"
                                : "px-1.5 py-0.5 bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-100 text-[#105B38] border border-emerald-200 dark:border-emerald-500/20"
                            )}
                            title={msg.isWholeDocument ? "Apply the updated draft" : "Replace full document"}
                          >
                            <RefreshCw className="w-3 h-3" />
                            <span>{msg.isWholeDocument ? "Apply" : "Replace"}</span>
                          </button>
                        </div>
                      </div>

                      <div className="p-2.5 rounded-lg bg-[#F8FAFC] dark:bg-[#0B131E] border border-[#E2E8F0] dark:border-[#1E2D44] font-serif text-[11px] text-[#1E293B] dark:text-[#F8FAFC] leading-relaxed whitespace-pre-wrap max-h-40 overflow-y-auto custom-scrollbar">
                        {msg.insertableClause}
                      </div>
                    </div>
                  )}

                  {/* Render Alternative Case Law Recommendations */}
                  {msg.recommendations && msg.recommendations.length > 0 && (
                    <div className="mt-3 space-y-2">
                      <div className="text-[10px] font-bold uppercase tracking-wider text-[#105B38] mb-1">
                        Alternative Case Law
                      </div>
                      {msg.recommendations.map((edit: any) => (
                        <div
                          key={edit.id}
                          className="w-full p-2.5 rounded-lg border border-[#105B38]/20 bg-[#105B38]/5"
                        >
                          <button
                            type="button"
                            onClick={() => {
                          ...
                          >
                            <div className="flex items-center justify-between gap-2">
                              <h4 className="text-xs font-bold text-[#0F172A] dark:text-[#F8FAFC]">{edit.title}</h4>
                              <span
                                className={cn(
                                  "text-[9px] font-bold uppercase px-1.5 py-0.5 rounded",
                                  edit.impact === "high" ? "bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-300" :
                                  edit.impact === "low" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300" :
                                  "bg-[#105B38]/10 text-[#105B38]"
                                )}
                              >
                                {edit.impact} impact
                              </span>
                            </div>
                            <p className="text-[10px] text-[#64748B] dark:text-[#94A3B8] leading-normal mt-1">{edit.reason}</p>
                          </button>
                            ...
                            <div className="mt-2 space-y-2 animate-in slide-in-from-top-1">
                              {edit.originalSnippet && (
                                <div className="rounded border border-rose-500/30 bg-rose-500/10 p-2">
                                  <p className="text-[9px] font-bold text-rose-700 dark:text-rose-300 uppercase tracking-wider mb-1">Before</p>
                                  <p className="text-[10px] text-[#0F172A] dark:text-[#F8FAFC] font-serif">{edit.originalSnippet}</p>
                                </div>
                              )}
                              <div className="rounded border border-emerald-500/30 bg-emerald-500/10 p-2">
                                <p className="text-[9px] font-bold text-emerald-700 dark:text-emerald-300 uppercase tracking-wider mb-1">After</p>
                                <p className="text-[10px] text-[#0F172A] dark:text-[#F8FAFC] font-serif">{edit.suggestedText}</p>
                              </div>
                              <div className="flex flex-col gap-1.5 mt-2 pt-2 border-t border-[#105B38]/10">
                                <button
                                  type="button"
                                  onClick={() => onInsertClause(edit.suggestedText, edit.title)}
                                  className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 rounded bg-[#105B38] text-white text-[10px] font-bold shadow-xs transition-colors hover:bg-[#0D4A2E]"
                                >
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleCopy(`rec-${edit.id}`, edit.suggestedText)}
                                  className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 rounded border border-[#E2E8F0] dark:border-[#1E2D44] bg-white dark:bg-[#131E2E] text-[#64748B] dark:text-[#94A3B8] text-[10px] font-bold transition-colors hover:bg-[#F8FAFC] dark:hover:bg-[#0B131E]"
                                >
                                  {copiedId === `rec-${edit.id}` ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                                  {copiedId === `rec-${edit.id}` ? "Copied" : "Copy to Clipboard"}
                                </button>
                              </div>
                            </div>
                          )}
      ...
      {activeTab === "tools" && (
        <div className="flex-1 p-3 overflow-y-auto custom-scrollbar space-y-4">
          ...
          <div className="p-3.5 rounded-xl bg-white dark:bg-[#131E2E] border border-[#E2E8F0] dark:border-[#1E2D44] shadow-xs space-y-3">
            ...
            <button
              type="button"
              disabled={riskScanStatus === "scanning"}
              onClick={async () => {
                if (!currentDocumentText.trim() || riskScanStatus === "scanning") return;
                setRiskScanStatus("scanning");
                try {
                  ...
                  setRiskScanResults(Array.isArray(data.risks) ? data.risks : []);
                  setRiskScanStatus("done");
                } catch {
                  setRiskScanResults([]);
                  setRiskScanStatus("error");
                }
              }}
              className="w-full py-2 rounded-xl bg-amber-50 dark:bg-amber-500/10 hover:bg-amber-100 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20 text-xs font-bold flex items-center justify-center gap-1.5 transition-colors disabled:opacity-60"
            >
              {riskScanStatus === "scanning" ? (
                <>
            ...
            <div className="pt-2">
              {riskScanStatus === "error" && (
                <div className="text-[11px] text-red-600 dark:text-red-400">Scan failed.</div>
              )}
              {riskScanStatus === "done" && riskScanResults.length === 0 && (
                <div className="text-[11px] text-emerald-600 dark:text-emerald-400 font-bold p-2 bg-emerald-50 dark:bg-emerald-500/10 rounded">
