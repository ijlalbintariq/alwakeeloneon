        ...
        <div className="flex-1 space-y-2">
          ...
          <p className="text-xs sm:text-sm text-[#334155] leading-relaxed font-sans">
            {isOverruled ? (
              <>
                ...
                <span className="underline decoration-rose-400 font-semibold text-rose-800">negative treatment (Overruled / Disapproved)</span>{" "}
              ...
              <>
                <strong className="text-amber-900 font-semibold">Notice for Counsel:</strong> This judgment has been{" "}
                <span className="underline decoration-amber-400 font-semibold text-amber-800">distinguished</span> in subsequent decisions on facts or constitutional grounds. Ensure factual parity before placing reliance.
              </>
            )}
          </p>
          ...
          <div className="pt-2 space-y-2">
            {(isOverruled ? overrulingCases : distinguishedCases).map((item, idx) => (
              <div
                key={item.id || idx}
                className="p-3 rounded-xl bg-white/95 border border-[#E2E8F0] hover:border-[#CBD5E1] transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-2 shadow-xs"
              >
                ...
                {item.linkedJudgmentId && onSelectJudgment && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-xl bg-[#F1F5F9] hover:bg-[#E2E8F0] text-[#0F172A] border border-[#CBD5E1] transition-all self-start sm:self-center flex-shrink-0"
                  >
