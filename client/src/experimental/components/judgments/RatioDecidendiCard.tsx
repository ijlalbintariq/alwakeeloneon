...
export const RatioDecidendiCard: React.FC<RatioDecidendiCardProps> = ({
  ...
  return (
    <div className="rounded-2xl border border-[#E2E8F0] dark:border-[#1E2D44] bg-white dark:bg-[#131E2E] p-5 sm:p-6 shadow-xs relative overflow-hidden space-y-4">
      {/* Subtle Background Accent */}
      <div className="absolute top-0 right-0 -mr-12 -mt-12 w-36 h-36 bg-emerald-50 dark:bg-emerald-500/10 rounded-full blur-2xl pointer-events-none" />

      {/* Header bar */}
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#E2E8F0] dark:border-[#1E2D44] pb-3.5">
        ...
        <button
          ...
          disabled={loading}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#F8FAFC] dark:bg-[#0B131E] hover:bg-[#F1F5F9] dark:bg-[#1E2D44] text-[#475569] hover:text-[#0F172A] dark:text-[#F8FAFC] text-xs font-semibold border border-[#E2E8F0] dark:border-[#1E2D44] transition-colors disabled:opacity-50"
      ...
      {loading && !summary ? (
        <div className="py-8 flex flex-col items-center justify-center gap-2 text-[#64748B] dark:text-[#94A3B8] dark:text-[#475569] font-mono text-xs">
          <Loader2 className="w-5 h-5 text-[#105B38] animate-spin" />
          <span>Synthesizing Ratio Decidendi & Key Holdings...</span>
        </div>
      ) : error && !summary ? (
        <div className="p-4 rounded-xl bg-[#F8FAFC] dark:bg-[#0B131E] border border-[#E2E8F0] dark:border-[#1E2D44] text-xs text-[#64748B] dark:text-[#94A3B8] dark:text-[#475569] flex items-center justify-between gap-3">
        ...
        <div className="space-y-4 text-xs sm:text-sm">
          {/* Outcome / Result */}
          {summary.result && (
            <div className="p-4 rounded-xl bg-emerald-50/6 dark:bg-emerald-500/100 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20/80 space-y-1.5">
              ...
              <p className="text-[#0F172A] dark:text-[#F8FAFC] leading-relaxed font-sans text-xs sm:text-sm font-medium">
                {summary.result}
              </p>
            </div>
          )}
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 gap-2">
                {summary.legalPrinciples.map((principle, idx) => (
                  <div
                    key={idx}
                    className="p-3 rounded-xl bg-[#F8FAFC] dark:bg-[#0B131E] border border-[#E2E8F0] dark:border-[#1E2D44] hover:border-[#CBD5E1] transition-all flex items-start gap-2.5 text-xs text-[#1E293B] leading-relaxed"
                  >
          ...
          {summary.significance && (
            <div className="p-3.5 rounded-xl bg-[#F8FAFC] dark:bg-[#0B131E] border border-[#E2E8F0] dark:border-[#1E2D44] text-xs text-[#475569] leading-relaxed font-sans">
              <strong className="text-[#0F172A] dark:text-[#F8FAFC] font-bold block mb-1 flex items-center gap-1.5">
