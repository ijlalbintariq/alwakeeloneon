...
function breaksEqual(left: LegalPaginationBreak[], right: LegalPaginationBreak[]): boolean {
  return left.length === right.length && left.every((item, index) => {
    ...
    return item.from === other?.from && item.kind === other.kind && Math.abs(item.gap - other.gap) <= 1;
  });
}
...
function computePaginationBreaks(view: EditorView, previous: LegalPaginationBreak[]): LegalPaginationBreak[] {
  ...
  const blocks: Array<{ from: number; nodeType: string; element: HTMLElement; height: number }> = [];
  ...
  for (let index = 0; index < blocks.length; index++) {
    ...
    const actualTop = rect.top - editorRect.top;
    const naturalTop = actualTop - previousGapBefore(block.from);
    ...
    if (naturalTop > pageContentStart + 1 && naturalTop + neededHeight > pageBottom) {
  ...
  editorElement.style.setProperty(
    "--legal-document-height",
    `${pageCount * pageHeight + Math.max(0, pageCount - 1) * pageGap}px`,
  );
  return nextBreaks;
}
