...
- **Turbo Mode**: Chat accepts optional `turbo: true` flag. Backend uses `gemini-3-pro-preview` for pro/enterprise users when turbo is enabled, `gemini-3-flash-preview` otherwise. Frontend shows Standard/Turbo toggle (locked for free tier)
