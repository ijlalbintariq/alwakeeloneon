  ...
  async downloadDocument(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        lastError = err;
        // If DNS fails (ENOTFOUND), don't retry repeatedly
    ...
    // Return empty buffer on failure instead of throwing unhandled exceptions
    console.warn(`[${this.courtName}] Notice: Could not reach ${doc.sourceUrl} (${lastError?.message})`);
    return { buffer: Buffer.alloc(0), mimeType: "text/html", hash: "" };
  }
