...
function format(text: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // 3. Very long lines imply it's a wrapped body paragraph (e.g. > 65 chars)
    // Most standard A4 PDF lines are 70-85 characters long. 
    // Most headers/titles are < 55 characters.
    const isShortHeader = lastAppendedOriginalLine.length < 55 && ![':', ',', ' '].includes(lastChar) && !/ (and|or|the|in|of|by|with|for)$/i.test(lastAppendedOriginalLine);
